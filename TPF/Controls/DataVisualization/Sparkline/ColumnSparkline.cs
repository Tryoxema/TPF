...
namespace TPF.Controls
{
    public class ColumnSparkline : SparklineBase
    {
        ...
        private ColumnsPanel _columnsPanel;
        ...
        protected override void OnUpdateIndicators()
        {
            ...
            foreach (ColumnItem child in _columnsPanel.Children)
            {
                ...
                {
                    child.SetBinding(BackgroundProperty, new Binding(nameof(FirstPointBrush)) { Source = this });
                    child.Type = IndicatorType.First;
                }
                else if (ShowLastPointIndicator && counter == DataPoints.Count - 1)
                {
                    child.SetBinding(BackgroundProperty, new Binding(nameof(LastPointBrush)) { Source = this });
                    child.Type = IndicatorType.Last;
                }
                else if (ShowHighPointIndicators && dataPoint.Y == maxValue)
                {
                    child.SetBinding(BackgroundProperty, new Binding(nameof(HighPointBrush)) { Source = this });
                    child.Type = IndicatorType.High;
                }
                else if (ShowLowPointIndicators && dataPoint.Y == minValue)
                {
                    child.SetBinding(BackgroundProperty, new Binding(nameof(LowPointBrush)) { Source = this });
                    child.Type = IndicatorType.Low;
                }
                else if (ShowNegativePointIndicators && dataPoint.Y < 0)
                {
                    child.SetBinding(BackgroundProperty, new Binding(nameof(NegativePointBrush)) { Source = this });
                    child.Type = IndicatorType.Negative;
                }
                else
                {
                    child.SetBinding(BackgroundProperty, new Binding(nameof(ColumnBrush)) { Source = this });
                    child.Type = IndicatorType.Normal;
                }
